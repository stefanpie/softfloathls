#include "softfloat_preprocessed.h"
...
public:
  ...
  hlsfloat_float(const float32 val) : data(val) {} // Added constructor
  ...
  hlsfloat_float sqrt() { return hlsfloat_float(float32_sqrt(data)); }
  ...
  bool operator==(const hlsfloat_float &d) { return float32_eq(data, d.data); }
  bool operator!=(const hlsfloat_float &d) { return !(*this == d); }
  bool operator<(const hlsfloat_float &d) { return float32_lt(data, d.data); }
  bool operator<=(const hlsfloat_float &d) { return float32_le(data, d.data); }
  bool operator>(const hlsfloat_float &d) { return !(*this <= d); }
  bool operator>=(const hlsfloat_float &d) { return !(*this < d); }

  //   conversions
  float to_float() { return float32_to_float(data); }
  int to_int32() { return float32_to_int32(data); }
  int to_int32_round_to_zero() { return float32_to_int32_round_to_zero(data); }
  long long to_int64() { return float32_to_int64(data); }
  long long to_int64_round_to_zero() {
    return float32_to_int64_round_to_zero(data);
  }
  float64 to_float64() { return float32_to_float64(data); }
  float128 to_float128() { return float32_to_float128(data); }
};